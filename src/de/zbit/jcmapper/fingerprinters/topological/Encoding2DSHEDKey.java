...
public class Encoding2DSHEDKey extends Encoding2D {
	...
	private HashMap<String, ArrayList<SHEDAtomContainer>> findPharmacophores(AtomContainer mol) {
	...
	private HashMap<String, Integer[]> calculateDistribution(
			HashMap<String, ArrayList<SHEDAtomContainer>> pharmacophores, AtomContainer mol) {
	...
	public List<IFeature> getFingerprint(IAtomContainer ac) {
		...
		HashMap<String, Integer[]> distribution = calculateDistribution(findPharmacophores((AtomContainer) ac),
				(AtomContainer) ac);
